...
export function removeDiacritics(org: string) {
	for(const d in diacritics)
		org = org.replace(diacritics[d], d);
	return org;
}

export function slugify(str: string, lower: boolean = true) {
	const rv = removeDiacritics(str)
		.trim()
		.replace(/['\s_]+/g, '-')
		.replace(/[^\w-]/g, '')
		.replace(/-+/g, '-')
		.replace(/^-+|-+$/g, '');
	return lower ? rv.toLowerCase() : rv;
}
...
/** Typescript helper */
export function Keys<T extends object>(o: T) { return <(keyof T)[]>Object.keys(o); }